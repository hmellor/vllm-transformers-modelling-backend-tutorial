...
All notebooks default to [OLMoE-1B-7B-Instruct](https://huggingface.co/allenai/OLMoE-1B-7B-0125-Instruct) (7B total parameters, 1B active per token). If you don't have enough memory, each notebook includes a commented-out fallback to [Qwen3-0.6B](https://huggingface.co/Qwen/Qwen3-0.6B) — swap the `MODEL_NAME` line to use it.

## Requirements

- Python 3.10+
- Memory (RAM for CPU inference, or VRAM on a CUDA GPU):
  - OLMoE-1B-7B-Instruct: ~14 GB for weights in bf16, plus a couple of GB for KV cache — budget ~16 GB
  - Qwen3-0.6B: ~1.2 GB for weights in bf16, plus a little for KV cache — budget ~2 GB
